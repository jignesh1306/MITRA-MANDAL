    ...
    <header className="sticky top-0 z-30 bg-white/95 backdrop-blur-md border-b border-gray-100 shadow-xs print:hidden">
      <div className="max-w-7xl mx-auto px-3 sm:px-6 lg:px-8">
        <div className="flex justify-between h-16 items-center">
          <Link to={user ? (user.role === 'ADMIN' ? '/admin' : '/member') : '/'} className="flex items-center gap-2 sm:gap-2.5">
            <img src="/logo.png" alt="Mitra-Mandal Logo" className="w-9 h-9 sm:w-10 sm:h-10 object-contain rounded-xl shadow-xs" />
            <div>
              <span className="text-lg sm:text-xl font-bold text-gray-900 tracking-tight font-gujarati">મિત્ર-મંડળ</span>
          ...
          <div className="flex items-center gap-2 sm:gap-3">
            <Link 
              to="/calculator" 
              className="hidden sm:flex items-center gap-1.5 px-3 py-1.5 text-xs font-semibold text-gray-700 hover:text-brand-600 bg-gray-100 hover:bg-brand-50 rounded-xl transition-all"
            >
